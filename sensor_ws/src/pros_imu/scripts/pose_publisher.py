    ...
    while not rospy.is_shutdown():
        try:
            ...
            rospy.loginfo("Thigh:x=%.2f, y=%.2f, z=%.2f;Knee:x=%.2f, y=%.2f, z=%.2f;Ankle:x=%.2f, y=%.2f, z=%.2f",
                            imu_thigh_buffer[6], imu_thigh_buffer[7], imu_thigh_buffer[8],
                            imu_knee_buffer[6], imu_knee_buffer[7], imu_knee_buffer[8],
                            imu_ankle_buffer[6], imu_ankle_buffer[7], imu_ankle_buffer[8])
            if use_memmap:
                imu_buffer[0] = time.time()-t0
